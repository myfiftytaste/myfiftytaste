# Display profile for picolifl

## Hero

- Archetype: The Demanding Explorer
- Subtitle: Exacting taste, wide horizons.
- One-liner: You wander far from the obvious picks, but you make every star work for it.
- Films detected: 49/50
- Profile quality: partial
- Social coverage: 45%
- Metadata coverage: 96%

## Radar scores

- mainstreamness: 4 / 5
- oldness: 4 / 5
- endurance: 3 / 5
- reviewness: 5 / 5

## Genre bubbles

- Drama: 37 (33%)
- Thriller: 13 (12%)
- History: 9 (8%)
- Comedy: 9 (8%)
- Romance: 9 (8%)
- Crime: 6 (5%)
- Mystery: 6 (5%)
- Science Fiction: 5 (5%)
- Animation: 3 (3%)
- Action: 3 (3%)
- Horror: 3 (3%)
- Adventure: 2 (2%)

## Country map

- France (FR): 25 (30%)
- USA (US): 20 (24%)
- UK (GB): 12 (14%)
- Belgium (BE): 7 (8%)
- Spain (ES): 4 (5%)
- Ireland (IE): 2 (2%)
- South Korea (KR): 2 (2%)
- Germany (DE): 2 (2%)
- Denmark (None): 1 (1%)
- Brazil (None): 1 (1%)

## Recommendations

- safe_pick: Secret Beyond the Door (0.4456)
- wild_card: Swapped (0.8268)
- deep_cut: The Baader Meinhof Complex (0.477)

## Cards

### Sévère

- Value: -0.37
- Label: 
- Description: Tu notes environ 0,37 étoile(s) en dessous de la moyenne Letterboxd.
- Confidence: medium
- Data source: 16 films comparables avec moyenne Letterboxd

### Reviewer

- Value: 48/49
- Label: 
- Description: Tu commentes souvent tes films vus.
- Confidence: medium
- Data source: 49 films depuis le RSS Letterboxd

### Durée moyenne

- Value: 1h57
- Label: 
- Description: Les films que tu as vus durent en moyenne 1h57.
- Confidence: high
- Data source: 46 films avec durée confirmée

### Chauvin

- Value: France
- Label: 
- Description: La France représente 30 % des occurrences de pays de production de tes films vus.
- Confidence: high
- Data source: 47 films avec métadonnées confirmées

## Synthesis cards audit

- rating gap method: paired_gap
- paired_count: 16
- runtime included count: 46
- runtime excluded count: 3

## Highlights

- most_niche: Summer of 85 (summer-of-85)
- most_mainstream: Inception (inception)
- most_cult: Paris, Texas (paris-texas)
- longest: The Hateful Eight (the-hateful-eight)
- shortest: The Speech (the-speech-2020-1)
- most_repeated_director: Joel Coen (2 films)

## Poster verification

- Posters verified: 8
- Posters ambiguous: 0
- Posters missing: 0
- Homonymy rejections: None

## Highlight metadata audit

- most_niche: Summer of 85 | slug: summer-of-85 | director: François Ozon | director_source: megabank | poster_status: verified | poster_source: letterboxd_rss | poster_url: yes
- most_mainstream: Inception | slug: inception | director: Christopher Nolan | director_source: megabank | poster_status: verified | poster_source: letterboxd_rss | poster_url: yes
- most_cult: Paris, Texas | slug: paris-texas | director: Wim Wenders | director_source: megabank | poster_status: verified | poster_source: letterboxd_rss | poster_url: yes
- longest: The Hateful Eight | slug: the-hateful-eight | director: Quentin Tarantino | director_source: megabank | poster_status: verified | poster_source: letterboxd_rss | poster_url: yes
- shortest: The Speech | slug: the-speech-2020-1 | director: Haohao Yan | director_source: tmdb_details_cache | poster_status: verified | poster_source: letterboxd_rss | poster_url: yes

## Warnings

- Profil calculé sur 49 films détectés. C’est assez pour esquisser une tendance, mais certaines conclusions peuvent encore bouger avec plus de films.
- Social stats are based only on films present in the Megabank.

## Config fallbacks

- None
